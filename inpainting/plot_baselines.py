...
figs_dir = "figs/baseline"
datpath = "dat/inpaint/baseline"
...
if __name__ == '__main__': 
    ...
    #         save_image(fwd.detach().cpu(), fout, nrow = 5)
        ...
        # opt = torch.optim.Adam([fwd], lr = 1e-2)
        ...
        # np.save(os.path.join(datpath, "adam_progression"), adamloss)

    
    true_min = np.load(os.path.join("dat","inpaint", "true_min_val.npy"))
    ...
    for nesterov_ss in ["1e-4", "2e-4", "5e-4", "1e-3", "2e-3", "5e-3"]:
        ...
        #     yk = yk.detach().requires_grad_(True)
        #     xklast = xk
        #     xk = yk - n_ss*recon_err_grad(yk)
        #     yk = xk + i/(i+3)*(xk-xklast)

        #     nesterovloss.append(recon_err(yk).item())
        # print("Nesterov {} final {:.6f}".format(nesterov_ss, nesterovloss[-1]))
        # np.save(os.path.join(datpath, "nesterov_progression_"+nesterov_ss), nesterovloss)
        nesterovloss = np.load(os.path.join(datpath, "nesterov_progression_"+nesterov_ss+".npy"))

        ax2.loglog(range(1, len(nesterovloss)),nesterovloss[1:] - true_min,  markevery=100, label=nesterov_ss)

    ax2.legend()
    ax2.axvline(2000, alpha=0.5, color='grey')
    fig2.savefig(os.path.join(figs_dir, "nesterov.pdf"))
    plt.close("all")
    ...
    for adam_ss in ["1e-3", "2e-3", "5e-3", "1e-2", "2e-2", "5e-2", "1e-1"]:
        ...
        # np.save(os.path.join(datpath, "adam_progression"+str(adam_ss)), adamloss)
        adamloss = np.load(os.path.join(datpath, "adam_progression"+adam_ss+".npy"))
        ax2.loglog(range(1, len(adamloss)),adamloss[1:] - true_min,  markevery=100, label=adam_ss)
    ax2.axvline(2000, alpha=0.5, color='grey')  
    ax2.legend()

    fig2.savefig(os.path.join(figs_dir, "adam.pdf"))
    plt.close("all")
    ...
    for gd_ss in ["2e-4", "5e-4", "1e-3", "2e-3", "5e-3", "1e-2", "2e-2", "5e-2", "1e-1"]:
        ...
        # np.save(os.path.join(datpath, "gd_progression"+str(gd_ss)), gdloss)
        gdloss = np.load(os.path.join(datpath, "gd_progression"+gd_ss+".npy"))
        ax2.loglog(range(1, len(gdloss)),gdloss[1:] - true_min,  markevery=100, label=gd_ss)
    ax2.axvline(2000, alpha=0.5, color='grey')
    ax2.legend()

    fig2.savefig(os.path.join(figs_dir, "gd.pdf"))
    plt.close("all")
