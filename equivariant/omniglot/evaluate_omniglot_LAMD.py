...
#%%
num_meta_batches = 50 # = 10000/16, loads 16 meta task at once
# num_meta_batches = 5
to_run = dict(LAMD = True,
                Reptile_LAMD = True)
# Baselines: SGD, Adam
    ...
    for meta_epoch, batch in tqdm.tqdm(zip(range(num_meta_batches), dataloader), total=num_meta_batches):
        ...
        for task_idx, (train_input, train_target, test_input,
            ...
            for i in range(lmd_hparams['num_optims']):
                # cap 15, sqrt works ok
                # cap 10, 0.25 works better
                # cap 20, 0.25 works better
                # cap 10, 0.75 pretty bad
                # cap 10, 0.15 very good
                # cap 100 constant 1/2: good (almost 1e-4)
                # cap 100 constant 1/3: worse than 1/2
                # cap = 10
                # if i < cap:
                #     t_k = lmd_model.stepsize[i]/2
                # else:
                #     t_k = t_k * np.sqrt(i/(i+1)) #np.sqrt(cap/i)
                cap = 100
                if i < cap:
                    t_k = lmd_model.stepsize[i]/1.5
                else:
                    t_k = t_k * np.power(i/(i+1),0.15)
                ...
                loss_list[i+1] = fwd_loss.item()
                accs_list[i+1] = test_acc(param_evaler.nn_fwd(fwd,train_input), train_target).item()
                taccs_list[i+1] = test_acc(param_evaler.nn_fwd(fwd,test_input), test_target).item()

            if accs_list[-1] == 1:
                LMD_accumulator.update(loss_list, accs_list, taccs_list)

    LMD_loss_list, LMD_accs_list, LMD_taccs_list = LMD_accumulator.vals()
    save_values("LAMD", LMD_loss_list, LMD_accs_list, LMD_taccs_list)
...
if to_run['Reptile_LAMD']:
    ...
    for meta_epoch, batch in tqdm.tqdm(zip(range(num_meta_batches), dataloader), total=num_meta_batches):
        ...
        for task_idx, (train_input, train_target, test_input,
            ...
            for i in range(lmd_hparams['num_optims']):
                # cap = 20
                # if i < cap:
                #     t_k = min(lmd_model.stepsize[i], t_k)
                # else:
                #     t_k = t_k * i/(i+1) #np.sqrt(cap/i) 
                # cap 100, /1.5 barely crosses the adam line
                # cap 10, 0.15 doesnt cross adam line
                cap = 100
                if i < cap:
                    t_k = lmd_model.stepsize[i]
                else:
                    t_k = t_k * np.power(i/(i+1),0.25)
                # cap = 100
                # if i < cap:
                #     t_k = lmd_model.stepsize[i]/1.5
                # else:
    ...
    save_values("Reptile_LAMD", Reptile_LMD_loss_list, Reptile_LMD_accs_list, Reptile_LMD_taccs_list)