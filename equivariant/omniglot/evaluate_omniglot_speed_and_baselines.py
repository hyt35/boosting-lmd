...
if to_run["SGD"]:
    print("SGD begin")
    start = time.time()
    for gd_ss in ["1e-3", "2e-3", "5e-3", "1e-2", "2e-2", "5e-2", "1e-1"]:
        ss = float(gd_ss)
        ...
        for meta_epoch, batch in tqdm.tqdm(zip(range(num_meta_batches), dataloader), total=num_meta_batches):
            ...
            for task_idx, (train_input, train_target, test_input,
                ...
                opt = torch.optim.SGD(model.parameters(), ss)
                model.train()
        ...
        save_values("SGD"+gd_ss, sgd_loss_list, sgd_accs_list, sgd_taccs_list)
    ...
    for adam_ss in ["1e-3", "2e-3", "5e-3", "1e-2", "2e-2", "5e-2", "1e-1"]:
        ...
        for meta_epoch, batch in tqdm.tqdm(zip(range(num_meta_batches), dataloader), total=num_meta_batches):
            ...
            for task_idx, (train_input, train_target, test_input,
                ...
                test_inputs, test_targets)):
                model = CNNModelSmall().to(device)
                opt = torch.optim.Adam(model.parameters(), ss)
                model.train()
                loss_list, accs_list, taccs_list = np.zeros(lmd_hparams['num_optims']+1),np.zeros(lmd_hparams['num_optims']+1),np.zeros(lmd_hparams['num_optims']+1)
                # loss_list[0] = lossFn(model(train_input), test_target).item()
                accs_list[0] = test_acc(model(train_input), train_target).item()
        ...
        save_values("Adam"+adam_ss, Adam_loss_list, Adam_accs_list, Adam_taccs_list)

    print("Adam time", time.time()-start)
...
if to_run["LMD"]:
    ...
    print("LMD time", time.time()-start)
#%% Reptile helpers
...
if to_run["Reptile_SGD"]:
    ...
    for gd_ss in ["1e-3", "2e-3", "5e-3", "1e-2", "2e-2", "5e-2", "1e-1"]:
        ...
        save_values("Reptile_SGD"+gd_ss, Reptile_sgd_loss_list, Reptile_sgd_accs_list, Reptile_sgd_taccs_list)
    print("Reptile_SGD time", time.time()-start)
...
if to_run['Reptile_Adam']:
    ...
    for adam_ss in ["1e-3", "2e-3", "5e-3", "1e-2", "2e-2", "5e-2", "1e-1"]:
        ...
        save_values("Reptile_Adam"+adam_ss, Reptile_Adam_loss_list, Reptile_Adam_accs_list, Reptile_Adam_taccs_list)
    print("Reptile_Adam total time", time.time()-start)
#%% Reptile + LMD
if to_run['Reptile_LMD']:
    ...
    print("Reptile_LMD time", time.time()-start)